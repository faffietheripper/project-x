...
export default function Header() {
  ...
  const [hasSystemNotifications, setHasSystemNotifications] = useState(false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (session?.user?.id) {
        ...
        // ✅ Fetch organisation using organisationId
        if (session.user.organisationId) {
          const org = await getOrganisationByUserId(
  ...
  const allowedChains = ["wastegenerator"];
  const chain = organisation?.chainOfCustody?.toLowerCase();
  const canCreateWaste = allowedChains.includes(chain);
