...
pub struct ProvisionOptionsInput {
    email: String,
    password: String,
}
...
async fn ensure_cloud_reachable(client: &Client) -> Result<(), String> {
    let response = client.get(format!("{}/api/desktop/v1/health", cloud_base_url())).send().await.map_err(|e| format!("Waste X Cloud is unreachable. Check the internet connection and configured Waste X Cloud address. {e}"))?;
...
#[tauri::command]
pub async fn desktop_provision_options(input: ProvisionOptionsInput) -> Result<Value, String> {
    ...
    let response = client
        .post(format!(
            "{}/api/desktop/v1/auth/provision-options",
            cloud_base_url()
        ))
        .json(&json!({
        ...
        .send()
        .await
        .map_err(|e| format!("Could not check Waste X Desktop access: {e}"))?;

    response_json("Desktop setup", response).await
}
...
pub fn desktop_provisioning_status(app: AppHandle) -> Result<ProvisioningStatus, String> {
    ...
    let default_site_name = match default_site_id.as_deref() {
        Some(site_id) => connection
            .query_row(
                "SELECT name FROM local_site WHERE id = ?1",
                [site_id],
    ...
    Ok(ProvisioningStatus {
        ...
        credentials_available: load_cloud_credentials().is_ok(),
    })
}
...
pub async fn desktop_provision_and_bootstrap(
    ...
    let default_site_id = input
        .default_site_id
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "Choose the Waste X operating site for this Desktop.".to_string())?;

    let client = Client::new();
    ensure_cloud_reachable(&client).await?;
    let response = client.post(format!("{}/api/desktop/v1/auth/provision", cloud_base_url())).json(&json!({
        "email": input.email.trim(), "password": input.password, "displayName": input.display_name.trim(), "platform": platform(), "defaultSiteId": default_site_id
    ...
    Ok(
        json!({ "ok": true, "deviceId": device.get("deviceId"), "organisationId": device.get("organisationId"), "bootstrap": bootstrap }),
    )
}
